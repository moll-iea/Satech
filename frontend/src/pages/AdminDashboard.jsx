...
export default function AdminDashboard() {
  ...
  const [exhibitionForm, setExhibitionForm] = useState(EMPTY_EXHIBITION_FORM);
  const [exhibitionEditingId, setExhibitionEditingId] = useState("");
  const [exhibitionImageFile, setExhibitionImageFile] = useState(null);
  const [exhibitionImagePreview, setExhibitionImagePreview] = useState("");
  const [exhibitionCurrentImage, setExhibitionCurrentImage] = useState("");
  ...
  const loadAdminData = async () => {
    ...
    try {
      ...
      const baseUrl = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
      const [messagesRes, productsRes, categoriesRes, exhibitionsRes] = await Promise.all([
      ...
      if (!exhibitionsRes.ok || !exhibitionsData.success) {
        throw new Error(exhibitionsData.message || "Failed to load exhibitions.");
      }
      ...
      setExhibitions(Array.isArray(exhibitionsData.data) ? exhibitionsData.data : []);
    } catch (err) {
  ...
  const reloadExhibitions = async () => {
    ...
    if (!response.ok || !payload.success) {
      throw new Error(payload.message || "Failed to reload exhibitions.");
    }
  ...
  const resetExhibitionForm = () => {
    setExhibitionForm(EMPTY_EXHIBITION_FORM);
    setExhibitionEditingId("");
    setExhibitionImageFile(null);
    setExhibitionCurrentImage("");
    setExhibitionError("");
  };

  const handleExhibitionImageChange = (event) => {
    const selectedFile = event.target.files && event.target.files[0] ? event.target.files[0] : null;
    setExhibitionImageFile(selectedFile);
    setExhibitionCurrentImage("");
  };

  const handleEditExhibition = (item) => {
    setExhibitionEditingId(item._id);
    setExhibitionForm({
      name: item.name || "",
      row: item.row || 1,
      order: item.order || 0,
    });
    setExhibitionCurrentImage(resolveImageUrl(item.image));
    setExhibitionImageFile(null);
    setExhibitionError("");
  };
  ...
  const handleSaveExhibition = async (event) => {
    ...
    if (!exhibitionForm.name.trim()) {
      setExhibitionError("Exhibition name is required.");
      return;
    }

    if (!exhibitionImageFile && !exhibitionCurrentImage) {
      setExhibitionError("Please select an image file.");
      return;
    }
    ...
    try {
      ...
      const baseUrl = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
      const isEditing = Boolean(exhibitionEditingId);
      const endpoint = isEditing ? `${baseUrl}/api/exhibitions/${exhibitionEditingId}` : `${baseUrl}/api/exhibitions`;
      const formData = new FormData();

      formData.append("name", exhibitionForm.name.trim());
      formData.append("row", exhibitionForm.row);
      formData.append("order", exhibitionForm.order);

      if (exhibitionImageFile) {
        formData.append("image", exhibitionImageFile);
      }
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message || "Failed to save exhibition.");
      }

      await reloadExhibitions();
      resetExhibitionForm();
    } catch (err) {
      setExhibitionError(err.message || "Failed to save exhibition.");
    } finally {
  ...
  const handleDeleteExhibition = async (id) => {
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message || "Failed to delete exhibition.");
      }

      await reloadExhibitions();
      if (exhibitionEditingId === id) {
        resetExhibitionForm();
      }
    } catch (err) {
      setExhibitionError(err.message || "Failed to delete exhibition.");
    } finally {
      setExhibitionLoading(false);
    }
  };
